...
namespace Software
{
    public partial class Diagnostics : Form
    {
        ...
        //Production Process
        ProdProcess OProdProcess = new ProdProcess();
        //Reports
        ...
        private void Diagnostics_FormClosed(object sender, EventArgs e)
        {
            ...
            //Control for the Instrument's Control as remote mode 
            btn_VisionSensorTrigRemote.Checked = true;
            //Control for the PCON Control as remote mode 
            btn_PCON_Remote.Checked = true;
            //Control for the Kistler Control as remote mode 
            btn_KistlerRemoteCtrl.Checked = true;
        }
        ...
        private void MachineStatus_Tick(object sender, EventArgs e)
        {
            ...
            //Limits
            txt_TorqueMin.Text = OSQLServer.Limits[(int)Prod_Limits.TorqueMin];
            txt_TorqueMax.Text = OSQLServer.Limits[(int)Prod_Limits.TorqueMax];
            txt_AngleMin.Text = OSQLServer.Limits[(int)Prod_Limits.AngleMin];
            txt_AngleMax.Text = OSQLServer.Limits[(int)Prod_Limits.AngleMax];
            //Clamp and Seating Point 
            txt_ClampMin.Text = OSQLServer.Limits[(int)Prod_Limits.ClampMin];
            txt_ClampMax.Text = OSQLServer.Limits[(int)Prod_Limits.ClampMax];
            txt_ClampAngleMin.Text = OSQLServer.Limits[(int)Prod_Limits.ClampAngleMin];
            txt_ClampAngleMax.Text = OSQLServer.Limits[(int)Prod_Limits.ClampAngleMax];
            txt_SPMin.Text = OSQLServer.Limits[(int)Prod_Limits.SeatingPointMin];
            txt_SPMax.Text = OSQLServer.Limits[(int)Prod_Limits.SeatingPointMax];
            #endregion
            ...
            //Limits
            txt_SpringVisionProg.Text = OSQLServer.Limits[(int)Prod_Limits.SpringVisionProg];
            txt_ArmatureVisionProg.Text = OSQLServer.Limits[(int)Prod_Limits.ArmatureVisionProg];
            #endregion
            ...
            //Control is enable to modify (Remote or Manual)
            pnl_KistlerRemoteCtrl.Enabled = OKistler.Parameters[(int)Instrument_Comm.CommStatus] && !HMI.OForm.MachineMode;
            #endregion
            ...
            #region Limits
            //Test 1
            txt_LimitChXmin.Text = OSQLServer.Limits[(int)Prod_Limits.Test1ChXmin];
            txt_LimitChXmax.Text = OSQLServer.Limits[(int)Prod_Limits.Test1ChXmax];
            txt_LimitChYmin.Text = OSQLServer.Limits[(int)Prod_Limits.Test1ChYmin];
            txt_LimitChYmax.Text = OSQLServer.Limits[(int)Prod_Limits.Test1ChYmax];
            //Test 2
            txt_LimitChXmin2.Text = OSQLServer.Limits[(int)Prod_Limits.Test2ChXmin];
            txt_LimitChXmax2.Text = OSQLServer.Limits[(int)Prod_Limits.Test2ChXmax];
            txt_LimitChYmin2.Text = OSQLServer.Limits[(int)Prod_Limits.Test2ChYmin];
            txt_LimitChYmax2.Text = OSQLServer.Limits[(int)Prod_Limits.Test2ChYmax];
            //Constant K
            txt_ConstantK.Text = OKistler.ConstantK.ToString("000.000");
            txt_ConstantKmin.Text = OSQLServer.Limits[(int)Prod_Limits.ConstantKmin];
            txt_ConstantKmax.Text = OSQLServer.Limits[(int)Prod_Limits.ConstantKmax];
            #endregion
            ...
            //Control is enable to modify (Remote or Manual)
            pnl_ServoPressControl.Enabled = OPCON.Parameters[(int)Instrument_Comm.CommStatus] && !HMI.OForm.MachineMode;
            #endregion
            ...
            if (!btn_PCON_Remote.Checked)
            {
                ...
                cbx_PCON_SON.Checked = btn_PCON_ServoON.Checked;
                //Servo ON?
                btn_PCON_Home.Visible = btn_PCON_ServoON.Checked;
                btn_PCON_Ready.Visible = btn_PCON_ServoON.Checked;
                btn_PCON_Standby.Visible = btn_PCON_ServoON.Checked;
                btn_PCON_Test1.Visible = btn_PCON_ServoON.Checked;
                btn_PCON_Test2.Visible = btn_PCON_ServoON.Checked;
                btn_Screwdriving.Visible = btn_PCON_ServoON.Checked;
            }
            else
            {
                ...
                btn_PCON_Home.Checked = cbx_PCON_Home.Checked;
                btn_PCON_ServoON.Visible = false;
                btn_PCON_ServoON.Checked = cbx_PCON_SON.Checked;
            ...
            #region Limits
            txt_LimitStandbyPos.Text = OSQLServer.Limits[(int)Prod_Limits.StandbyPos];
            txt_LimitStandbySpeed.Text = OSQLServer.Limits[(int)Prod_Limits.StandbySpeed];
            txt_LimitReadyPos.Text = OSQLServer.Limits[(int)Prod_Limits.ReadyPos];
            txt_LimitReadySpeed.Text = OSQLServer.Limits[(int)Prod_Limits.ReadySpeed];
            txt_LimitTest1Pos.Text = OSQLServer.Limits[(int)Prod_Limits.Test1Pos];
            txt_LimitTest1Speed.Text = OSQLServer.Limits[(int)Prod_Limits.Test1Speed];
            txt_LimitTest2Pos.Text = OSQLServer.Limits[(int)Prod_Limits.Test2Pos];
            txt_LimitTest2Speed.Text = OSQLServer.Limits[(int)Prod_Limits.Test2Speed];
            txt_LimitScrewdrivingPos.Text = OSQLServer.Limits[(int)Prod_Limits.ScrewdrivingPos];
            txt_LimitScrewdrivingSpeed.Text = OSQLServer.Limits[(int)Prod_Limits.ScrewdrivingSpeed];
            #endregion
        ...
        {
            DialogResult Answer;
            Answer = MessageBox.Show("Be careful, the servo press going to move", "PCON: Standby Position", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
        ...
        private void btn_PCON_Ready_Click_1(object sender, EventArgs e)
        {
            DialogResult Answer;
            Answer = MessageBox.Show("Be careful, the servo press going to move", "PCON: Ready Position", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (Answer == DialogResult.OK)
            {
                tskReadyPos();
            }
        ...
        {
            DialogResult Answer;
            Answer = MessageBox.Show("Be careful, the servo press going to move", "PCON: Test 1 Position", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (Answer == DialogResult.OK)
            {
                tskTest1Pos();
            }
        ...
        {
            DialogResult Answer;
            Answer = MessageBox.Show("Be careful, the servo press going to move", "PCON: Test 2 Position", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (Answer == DialogResult.OK)
            {
                tskTest2Pos();
            }
        ...
        {
            DialogResult Answer;
            Answer = MessageBox.Show("Be careful, the servo press going to move", "PCON: Screwdriving Position", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (Answer == DialogResult.OK)
            {
                tskScrewdrivingPos();
            }
        ...
        //Standby Position
        private async Task tskStandbyPos()
        {
            await Task.Run(() => StandbyPos());
        }
        private void StandbyPos()
        {
            OProdProcess.StandbyPosition();
        }
        //Ready Position
        private async Task tskReadyPos()
        {
            await Task.Run(() => ReadyPos());
        }
        private void ReadyPos()
        {
            OProdProcess.ReadyPosition();
        }
        //Test 1 Position
        private async Task tskTest1Pos()
        {
            await Task.Run(() => Test1Pos());
        }
        private void Test1Pos()
        {
            OProdProcess.Test1Position();
        }
        //Test 2 Position
        private async Task tskTest2Pos()
        {
            await Task.Run(() => Test2Pos());
        }
        private void Test2Pos()
        {
            OProdProcess.Test2Position();
        }
        //Screwdriving Position
        private async Task tskScrewdrivingPos()
        {
            await Task.Run(() => ScrewdrivingPos());
        }
        private void ScrewdrivingPos()
        {
            OProdProcess.ScrewdrivingPosition();
        }
        #endregion
